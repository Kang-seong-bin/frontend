document.getElementById('search').addEventListener('click', function(){
    ...
    if(query=="") alert('검색어는 필수 입력란입니다.')          //카카오 open api 요청시 필수 파라미터
    search(query,page,size,sort)        //새로운 검색을 위한 함수 호출

})      //함수 끝
// search('트와이스',1,10,accuracy) //함수 정의보다 앞에서 호출하면 오류 -> 맨마지막줄 함수 뒤로 이동하면 오류가 발생하지 않는다.
...
const search = function(v1, v2, v3, v4){
            ...
            //2.요청header 설정
            xhr.setRequestHeader("Authorization","KakaoAK 070eb5f72c7c497ff5bc49cdf2681f3c");
 ...
 search('에스파',1,10,'accuracy') //함수 호출
